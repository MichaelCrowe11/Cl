...
from enhanced_vision_validator import EnhancedVisionValidator, validate_uploaded_specimen_image
...
def process_image():
    ...
    try:
        # Enhanced AI vision validation and scientific analysis
        specimen_metadata = {
            'specimen_type': request.form.get('specimen_type', 'fresh'),
            'species': request.form.get('claimed_species', ''),
            'location': request.form.get('location', ''),
            'collection_date': request.form.get('collection_date', ''),
            'description': request.form.get('sample_description', ''),
            'season': request.form.get('season', '')
        }
        
        # Run OpenAI vision validation first
        logger.info(f"Starting enhanced AI vision validation for {sample_name}")
        ai_validation_results = validate_uploaded_specimen_image(filepath, specimen_metadata)
        
        # Enhanced identification for species analysis
        if analyze_species:
            # Create context for better identification
            context = {
                'sample_type': specimen_metadata.get('specimen_type', 'dried_specimen'),
                'user_notes': specimen_metadata.get('description', ''),
                'location': specimen_metadata.get('location', ''),
                'season': specimen_metadata.get('season', '')
            }
            ...
            results['ai_validation'] = ai_validation_results
            
            # Premium dried specimen analysis if applicable
            if specimen_metadata.get('specimen_type') == 'dried' and specimen_metadata.get('species'):
                logger.info(f"Running premium dried specimen analysis")
                validator = EnhancedVisionValidator()
                premium_analysis = validator.validate_dried_specimen_image(
                    filepath, specimen_metadata.get('species')
                )
                results['premium_dried_analysis'] = premium_analysis
            
        else:
            # Standard analysis with AI validation
            cv_results = computer_vision.process_sample_image(
                image_path=filepath,
                output_dir=analysis_dir,
                analyze_species=analyze_species,
